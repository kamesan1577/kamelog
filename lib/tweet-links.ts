  ...
  while (changed && result) {
    ...
    for (const [open, close] of pairs) {
      if (
        ...
        count(result, close) > count(result, open)
      ) {
        result = result.slice(0, -1).replace(SIMPLE_TRAILING_PUNCTUATION, "");
        changed = true;
      }
    }
  }
...
export function firstHttpUrl(text: string) {
  return (
    splitTweetText(text).find((part) => part.type === "url")?.value ?? null
  );
}
