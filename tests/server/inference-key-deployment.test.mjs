...
const updater = await readFile(new URL("../../ops/kamelog-update", import.meta.url), "utf8");
...
test("host updater initializes the key before skipping an unchanged release", () => {
  ...
  assert.match(compose, /KAMELOG_INFERENCE_ENCRYPTION_KEY: \$\{KAMELOG_INFERENCE_ENCRYPTION_KEY:-\}/);
  assert.doesNotMatch(compose, /KAMELOG_INFERENCE_ENCRYPTION_KEY:\s*[^\n]*build:/);
});
...
test("key initialization is idempotent and does not print its secret", () => {
  ...
  try {
    ...
    const output = execFileSync("bash", ["-c", shell], { env: { ...process.env, ENV_FILE: envFile }, encoding: "utf8" });
    const content = readFileSync(envFile, "utf8");
    const matches = [...content.matchAll(/^KAMELOG_INFERENCE_ENCRYPTION_KEY=(.+)$/gm)];
