  ...
  await fixture(async (store) => {
    ...
    const fake = {
      ...
      async inferTags({ post, candidates }) {
        ...
        return {
          status: "classified",
          tags: post.id === "target" ? [{ tag: "Go" }] : [],
        };
      },
    };
    assert.equal(
      (await autoTagPosts(store, { tagInference: fake })).processed,
      2,
    );
    ...
    assert.equal(
      store.db
        .prepare("SELECT COUNT(*) AS n FROM inference_runs WHERE task='tag'")
        .get().n,
      2,
    );
    assert.equal(
      (await autoTagPosts(store, { tagInference: fake })).skipped,
      2,
    );
...
test("abstentions, invalid candidates and failures preserve old tags for retry", async () => {
  await fixture(async (store) => {
    ...
    const retry = await autoTagPosts(store, {
      tagInference: {
        ...fixtureModel,
        inferTags: async () => ({ status: "classified", tags: [] }),
      },
    });
    assert.equal(retry.processed, 1);
    assert.deepEqual(store.get("posts", "target").tags, []);
    assert.equal(
      (await autoTagPosts(store, { tagInference: fixtureModel })).skipped,
      2,
    );
  });
});
...
test("candidate shortlist keeps an exact alias match despite its ranking limit", () => {
  ...
  const selected = selectTagCandidates(
    { title: "", body: "Golangで作った" },
    candidates,
    10,
  );
...
test("tag toggle is independent and disables all inference calls", async () => {
  await fixture(async (store) => {
    ...
    assert.equal(
      (await autoTagPosts(store, { tagInference: fake, requireEnabled: true }))
        .disabled,
      true,
    );
    assert.equal(called, 0);
    store.saveInferenceSettings({
      autoTagEnabled: true,
      autoThreadEnabled: false,
    });
    const enabled = await autoTagPosts(store, {
      tagInference: fake,
      requireEnabled: true,
    });
