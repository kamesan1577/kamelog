...
The legacy allowlist is now empty; the shared styles have moved into the Design System style layer, while selector-level extraction from `styles/globals.css` remains in progress. The current implementation has completed the MediaGallery, Markdown help, Markdown embeds, ProjectList, Vlog display/recording controls, preview shell, app-shell coordinate system, public header/navigation, mobile navigation/create controls, home search, editor media, detail, reader, sidebar, editor, composer, timeline, and post-pattern, and dialog-state extractions. Remaining global responsibilities are final editor auxiliary rules and duplicate/orphan cleanup; these remain tracked as migration work rather than being treated as complete.
