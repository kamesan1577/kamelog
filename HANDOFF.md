...
- 2026-09-08 Issue #56の再調整では、青を主要色から外し、元の「タイムラインを見る」の墨色 `#37352f` を共通ボタン・選択状態・focus・リンクへ展開した。暖色の地と白い情報面、1px罫線でトップページとタイムラインを再構成し、`make check` とPR CIで検証する。ローカル `make e2e` はPlaywright Chromiumを取得できないため、PC/390pxの画面回帰とスクリーンショットはPR CIで確認する。
