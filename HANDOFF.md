...
- ホーム最下部の開発情報フッターに相互リンク欄を追加し、Shiryu のホームページへの200×40配布バナーリンクを表示する。バナーは `https://shiryu.win/` から同一origin制限付きで検出・取得し、1日キャッシュする。
...
- 2026-09-10 相互リンク変更では、Shiryu配布バナーの相対URL解決・候補選択・外部origin拒否をunit testへ追加した。この実行環境ではリポジトリをローカルcheckoutしていないため `make check` / `make e2e` は未実施で、PRのCI `check` / `e2e` / `container` で確認する。
