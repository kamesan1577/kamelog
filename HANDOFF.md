...
2026-09-20 Issue #245: inferred tweet parents are stored independently from explicit reply parents and read through `effectiveParentId`. The pending worker only considers up to five earlier owner tweets from the preceding 30 minutes; it remains disabled until owner inference settings enable it and a Jev credential is configured. Unit coverage verifies FakeThreadInference injection and manual-parent protection. Full CI and host timer installation are pending.

2026-09-20 Issue #243 implementation in progress: provider-neutral `TagInference` / `ThreadInference` ports isolate inference requests and responses from application/domain code. Existing local tag classification now runs through the tag port, and Jev adapters are isolated under `server/inference`. Owner-only inference status, independent tag/thread ON/OFF, and encrypted Jev credential registration are available; credentials use AES-256-GCM with a DB-external `KAMELOG_INFERENCE_ENCRYPTION_KEY` and are never returned by GET. Schema version 10 makes `post_tags.confidence` nullable so uncalibrated provider outputs are not invented as probabilities, and adds durable generic inference job/run storage. Unit tests cover fake-port candidate validation and encrypted credential API boundaries. Full `npm test` passes after the schema migration fixture update; remaining check/e2e/container verification is pending.

2026-09-17 Issue #121 implementation in progress: reply drafts now retain `parentId`, federated reply Notes expose `inReplyTo`, replies cannot newly enable federation when their parent is private, and the public timeline groups matching tweet threads with parent-first context. Inline tweets, modal tweets, and replies now share the `tweetPostInput` payload contract. The reply detail form supports up to four images through the shared paste/drop bridge, X intent, Fediverse selection, reply-draft restore, and explicit close/save/discard handling. The reply E2E covers real image selection, X Intent, Escape, outside-click unsaved protection, and parent-child rendering. The former DOM/portal thread bridge is now Notebook-owned direct rendering (`TweetThreadDetail`) with parent → current → child order, and owner/public Storybook stories build successfully. Host `make check` is 85/86 only because the host lacks FFmpeg; the CI-equivalent Playwright Noble container with FFmpeg and safe git workspace passes the full server suite 86/86, production build, Storybook build, Storybook component tests 31/31 (40 tests), and complete E2E 40/40. Full CI has not been triggered remotely and no deployment was performed.
...
2026-09-17 Issue #99 visual baseline states are now committed for PageHeader, PostCard, ProjectList, and MobileNavigation at 390, 768, and 1440 widths, alongside the existing Button baselines. The images were generated by CI Chromium and will be checked unchanged on the next run.
