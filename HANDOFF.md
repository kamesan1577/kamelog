...
- ブログ詳細では、いいね・リンクコピー・X共有を記事コンテンツより前へ移し、タイムライン側の操作は維持する。
...
- 2026-09-07 Issue #41のブログ詳細アクション配置変更は、接続環境でローカル `make check` / `make e2e` を実行できないため未検証。CSS契約テストと1280px/390pxのPlaywright回帰を追加し、PR CIで確認する。
