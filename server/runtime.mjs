  ...
  try {
    ...
    if (service === "api" && response.status === 400) {
      try {
        ...
        unexpected = body?.error === "操作に失敗しました。入力を保持して再試行してください。";
      } catch { /* Non-JSON responses are not errors by themselves. */ }
    }
    if (unexpected)
      void reportNotification(getStore(), config.inferenceEncryptionKey,
        { service, level: "error", code: "unexpected_failure" });
    return response;
  } catch (error) {
    ...
    void reportNotification(getStore(), config.inferenceEncryptionKey,
      { service, level: "critical", code: "unhandled_exception" });
    throw error;
  }
}
...
export function handleActivityPub(request) {
  const config = configuration();
  return monitored(request, "activitypub", createActivityPubHandler(getStore(), config), config);
}
