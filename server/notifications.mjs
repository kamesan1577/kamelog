...
export const NOTIFICATION_SERVICES = Object.freeze(["api", "activitypub", "process"]);
export const NOTIFICATION_LEVELS = Object.freeze(["debug", "info", "warn", "error", "critical"]);
export const NOTIFICATION_PROVIDERS = Object.freeze(["discord", "slack", "generic"]);
...
export function notificationSettings(store, encryptionKey) {
  ...
  const { enabled, provider, minimumLevel, services } = { ...defaults, ...saved };
  return {
    enabled,
    provider,
    minimumLevel,
...
function publicAddress(address, family) {
  return (family === 4 || family === 6) &&
    ...
    (family === 4 || address.toLowerCase().startsWith("2") || address.toLowerCase().startsWith("3"));
}

export function validateWebhookUrl(value, provider) {
  if (typeof value !== "string" || value.length > 2048) throw new TypeError("Invalid webhook URL");
  let url;
  try { url = new URL(value); } catch { throw new TypeError("Invalid webhook URL"); }
  ...
  if (provider === "discord" && !(
    ...
    /^\/api\/webhooks\/\d+\/[A-Za-z0-9_-]+\/?$/.test(url.pathname) && !url.search
  )) throw new TypeError("Invalid Discord webhook URL");
  if (provider === "slack" && !(url.hostname === "hooks.slack.com" &&
    /^\/services\/[A-Za-z0-9/_-]+$/.test(url.pathname) && !url.search))
    throw new TypeError("Invalid Slack webhook URL");
  if (provider === "generic" && (!/^[a-z0-9.-]+$/i.test(url.hostname) ||
...
export function updateNotificationSettings(store, encryptionKey, input) {
  if (!input || typeof input !== "object" || Array.isArray(input) ||
      Object.keys(input).some((key) => !["enabled", "provider", "minimumLevel", "services", "webhookUrl"].includes(key)))
  ...
  if (typeof next.enabled !== "boolean" || !NOTIFICATION_PROVIDERS.includes(next.provider) ||
      !NOTIFICATION_LEVELS.includes(next.minimumLevel) || !Array.isArray(next.services) ||
  ...
  if (hasUrl) {
    ...
    encrypted = box.encrypt(validateWebhookUrl(input.webhookUrl, next.provider));
  }
  if (next.enabled && !(hasUrl || (next.provider === current.provider && current.webhookConfigured)))
  ...
  store.transaction(() => {
    ...
    if (hasUrl) store.save("settings", CREDENTIAL_ID, { provider: next.provider, encrypted });
...
export function clearNotificationWebhook(store, encryptionKey) {
  store.transaction(() => {
    store.remove("settings", CREDENTIAL_ID);
    store.save("settings", SETTINGS_ID, { ...notificationSettings(store, encryptionKey), enabled: false,
      webhookConfigured: undefined, encryptionAvailable: undefined });
  });
  return notificationSettings(store, encryptionKey);
}
    ...
    const request = httpsRequest(url, {
      ...
      lookup(hostname, options, callback) {
        dnsLookup(hostname, { all: true }, (error, addresses) => {
          if (error || !addresses?.length || addresses.some(({ address, family }) => !publicAddress(address, family))) {
...
export const notificationDestinations = Object.freeze({
  discord: (url, event, send = sendHttps) => send(url, { content: `[${event.level.toUpperCase()}] ${event.service}: ${event.message} (${event.code}) · ${event.timestamp}`, allowed_mentions: { parse: [] } }),
  slack: (url, event, send = sendHttps) => send(url, { text: `[${event.level.toUpperCase()}] ${event.service}: ${event.message} (${event.code}) · ${event.timestamp}` }),
  generic: (url, event, send = sendHttps) => send(url, event),
});

const recent = new Map();
let inFlight = 0;
export async function reportNotification(store, encryptionKey, event, { send = sendHttps, force = false } = {}) {
  try {
    const settings = notificationSettings(store, encryptionKey);
    if ((!settings.enabled && !force) || !settings.webhookConfigured ||
        (!force && (!settings.services.includes(event.service) ||
        NOTIFICATION_LEVELS.indexOf(event.level) < NOTIFICATION_LEVELS.indexOf(settings.minimumLevel))))
      return false;
    if (!NOTIFICATION_SERVICES.includes(event.service) || !NOTIFICATION_LEVELS.includes(event.level) ||
        !Object.hasOwn(messages, event.code) || inFlight >= 8) return false;
    ...
    if (!box) return false;
    const credential = store.get("settings", CREDENTIAL_ID);
    const url = validateWebhookUrl(box.decrypt(credential.encrypted), settings.provider);
    const payload = { timestamp: new Date(now).toISOString(), level: event.level,
      service: event.service, code: event.code, message: messages[event.code] };
    recent.set(key, now);
    if (recent.size > 256) recent.clear();
    inFlight++;
    try { await notificationDestinations[settings.provider](url, payload, send); return true; }
    finally { inFlight--; }
  } catch {
