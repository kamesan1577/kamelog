...
export function NotificationSettingsLink() {
  ...
  useEffect(() => {
    const sync = () => {
      const node = document.querySelector<HTMLElement>('[data-ds="owner-inference"]');
  ...
  return target ? createPortal(
    <a href="/settings/notifications" data-ds="owner-notification-link"
      ...
      <Bell size={16} aria-hidden="true" /> サーバーエラーの通知設定 <ArrowUpRight size={14} aria-hidden="true" />
    </a>, target,
  ) : null;
}
